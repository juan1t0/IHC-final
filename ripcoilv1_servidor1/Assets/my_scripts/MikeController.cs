...
public class MikeController : MonoBehaviour
{
    ...
    void Start()
    {
        if (throwableObject)
        {
            initialRotation = throwableObject.rotation;
        }
...
//            Debug.Log("dentro del update,if: " + userId);
            switch (currentState)
            {
                case BallState.Hidden:
                    UpdateBallHide();
//                    serverScript.sendDataS(PosToString(throwableObject.position), "mike");
                    // try to catch
        ...
        {
            currentState = BallState.Hidden;//infoText.text = currentState.ToString();
        }
    }
    ...
    private void UpdateBallHide()
    {
        if (throwableObject)
        {
            throwableObject.position = new Vector3(0, 40, -271);
            throwableObject.rotation = initialRotation;
    ...
    private void UpdateHandRaise()
    {
        ...
        if (jointIndex >= 0 && throwableObject)
        {
            throwableObject.position = vHandPos;
    ...
    private void UpdateBallThrow()
    {
        ...
        if (jointIndex >= 0 && manager.IsJointTracked(userId, jointIndex) && GetJointPositionInv(userId, jointIndex).y >= lowestPos.y)
        {
            ...
            throwableObject.position = handPos2 + (new Vector3(0,-20,-271));
  ...
  //              Debug.Log(string.Format("Dist: {0:F3}; Time: {1:F3}; Velocity: {2:F3}", throwDist, throwTime, velocity));
                ...
                if (throwableObject)
                {
                    throwableObject.forward = throwDir.normalized;
