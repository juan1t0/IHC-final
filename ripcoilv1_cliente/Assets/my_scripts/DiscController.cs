...
public class DiscController : MonoBehaviour
{
    ...
    void Update()
    {
        if (client.stream.DataAvailable)
        {
            ...
            if (data != null)
            {
                ballObject.position = StringToVector(data);
            }
    ...
    private void UpdateBallHide()
    {
        if (ballObject)
        {
            ballObject.position = new Vector3(0, 40, -375);
    ...
    private void UpdateHandRaise()
    {
        ...
        {
            ballObject.position = new Vector3(vHandPos.x + (0f), vHandPos.y + (-22f), vHandPos.z + (-400));
    ...
    private void UpdateBallThrow()
    {
        ...
        if (jointIndex >= 0 && manager.IsJointTracked(userId, jointIndex) /*&& GetJointPositionInv(userId, jointIndex).y >= lowestPos.y*/)
        {
            ...
            ballObject.position = new Vector3(handPos2.x + (0f), handPos2.y + (56f), handPos2.z + (-380));
